...
program
  ...
  .option('-o, --output <output>', '自定义输出内容: tree: 依赖树, list: 依赖列表, useless: 非依赖树, 不写默认全选')
...
if (program.output.tree) {
  console.log('// 依赖树');
  console.log(JSON.stringify(utils.pathTree, (key, value) => {
...
if (program.output.useless) {
  ...
  console.log(JSON.stringify(useless, (key, value) => {
    if (!Object.keys(value).length) {
      return '';
    } else {
      return value;
